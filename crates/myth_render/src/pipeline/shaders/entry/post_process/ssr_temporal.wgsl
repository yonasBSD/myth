...
@group(1) @binding(1) var t_history_reflection: texture_2d<f32>;
@group(1) @binding(2) var t_depth: texture_depth_2d;
@group(1) @binding(3) var t_normal: texture_2d<f32>;
@group(1) @binding(4) var t_history_meta: texture_2d<f32>;
@group(1) @binding(5) var t_velocity: texture_2d<f32>;
@group(1) @binding(6) var t_material_data: texture_2d<f32>;
@group(1) @binding(7) var s_linear: sampler;
@group(1) @binding(8) var s_point: sampler;
@group(1) @binding(9) var<uniform> u_ssr: SsrUniforms;
...
fn sample_valid_history(
    ...
    let normal_threshold = u_ssr.reprojection_params.y;
    let depth_threshold = u_ssr.reprojection_params.z;
    ...
    for (var y: i32 = 0; y <= 1; y++) {
        for (var x: i32 = 0; x <= 1; x++) {
            ...
            if (dot(validation_world_normal, hist_normal) < normal_threshold
                || abs(expected_prev_linear - hist_linear)
                    > depth_threshold * max(expected_prev_linear, 1.0)
...
fn fs_main(in: VertexOutput) -> TemporalOutput {
    ...
    if (!camera_cut && (u_ssr.frame_params.w & 1u) != 0u) {
        ...
        let history_uv = jitter_history_uv(surface_uv - velocity);
        if (history_uv.x >= 0.0 && history_uv.x <= 1.0
            ...
            if (history.valid) {
                ...
                let box_extent = max(
                    std_dev * u_ssr.temporal_params.x,
                    vec3<f32>(1e-4)
                );
